...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
  ...
  {
    flagVolume = 4;
    if ((step->IsFirstStepInVolume())&&(step->GetTrack()->GetTrackID()==1)&&(false==fpEventAction->GetStartTrackFound()))
    {fpEventAction->SetStartTrack(step->GetPreStepPoint()->GetPosition());fpEventAction->SetStartTrackFound();}
    if ((step->IsLastStepInVolume())&&(step->GetTrack()->GetTrackID()==1))
    {fpEventAction->SetEndTrack(step->GetPostStepPoint()->GetPosition());}
  }
  ...
  if ((volumeName!="world")&&(volumeName!="waterBox")&&(volumeName!="TrackingVol")&&(dE!=0))
  {
    ...
    if ((volumeName!="world")&&(volumeName!="waterBox"))
  {

    fpEventAction->AddEdepTracking(dE);
  ...
  if ((flagVolume != 3) && (dE > 0))
{
    ...
    G4int copyNum = step->GetPreStepPoint()->GetTouchableHandle()->GetReplicaNumber();
...
G4ThreeVector point = prePoint + G4UniformRand()*(postPoint - prePoint);  
    ...
    analysisManager->FillNtupleIColumn(1, 1, copyNum);
    ...
    analysisManager->FillNtupleIColumn(1, 6, flagVolume);
